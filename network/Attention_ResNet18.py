# -*- coding:utf-8 -*-
'''
@Updatingtime: 2021/9/29 11:24
@Author      : Yilan Zhang
@Filename    : Attention_ResNet18.py
...
class BasicBlock_HDCSA(nn.Module):
    expansion = 1

    def __init__(self, inplanes, planes, stride=1, downsample=None):
        super(BasicBlock_HDCSA, self).__init__()
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=4, hash_bit=32):
        self.inplanes = 64
    ...
    def forward(self, x):
        ...
        x = self.avgpool(x)
        x = x.view(x.size(0), -1)
        x = self.hash_layer(x) #hashing layer
...
def resnet18(pretrained=False, **kwargs):
    ...
    """
    model = ResNet(BasicBlock_HDCSA, [2, 2, 2, 2], num_classes=4, hash_bit=16, **kwargs)
    if pretrained:
